...
export async function POST(request: Request) {
  try {
    ...
    // Update listing status to NEEDS_FIX and store feedback
    ...
    await supabaseAdmin.from('notifications').insert({
      ...
      type: 'INFO',
      title: 'Action Required: Fix Listing',
      message: `Admin requested changes for "${product.basic_info?.title || 'Draft'}": ${reason}`
    });
