...
async function run() {
  const { data: prod, error: uErr } = await supabase.from('products').select('supplier_id').limit(1);
  if (uErr) {
    console.error('Error fetching prod:', uErr);
    return;
  }
  
  const supplierId = prod[0]?.supplier_id;
  if (!supplierId) {
