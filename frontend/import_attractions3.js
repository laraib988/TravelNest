...
async function run() {
  const { data: users, error: err } = await supabase.from('supplier_profiles').select('id').limit(1);
  if (err) { console.error(err); }
  
  let supplierId = users && users[0] ? users[0].id : null;
  
  if (!supplierId) {
     const { data: authUsers } = await supabase.auth.admin.listUsers();
     if(authUsers && authUsers.users && authUsers.users.length > 0) {
       supplierId = authUsers.users[0].id;
     } else {
       console.log('No users at all, creating one...');
       const { data: newUser, error: newErr } = await supabase.auth.admin.createUser({
          email: 'admin_tickets@vaitour.com',
          password: 'password123',
          email_confirm: true
       });
       if(newErr) { console.error('Error creating user:', newErr); return; }
